...
SHEET_TITLE = 'Antam_Retro_Potrait';
...
fetch(FULL_URL)
  ...
  .then(rep => {
    ...
    let rp_wrapper = document.getElementById('rp-wrapper');

    let rp_grid = document.createElement('div');
    rp_grid.className = 'product-grid swiper-slide';
    rp_wrapper.appendChild(rp_grid);
    ...
    for(let i = 0; i<length; i++){
      ...
      if(row.c[0] && row.c[0].v && row.c[1] && row.c[1].v && row.c[2] && row.c[2].v){
        ...
        rp_grid.appendChild(product_card);
        product_count++;
        ...
        if(product_count >= 6){
          if(length > 7){
            console.log(length);
            rp_grid = document.createElement('div');
            rp_grid.className = 'product-grid swiper-slide';
            rp_wrapper.appendChild(rp_grid);
            product_count = 0;
          }
