...
fetch(FULL_URL)
  ...
  .then(rep => {
    ...
    let figura_wrapper = document.getElementById('figura-wrapper');

    let figura_grid = document.createElement('div');
    figura_grid.className = 'product-grid swiper-slide';
    figura_wrapper.appendChild(figura_grid);
    ...
    for(let i = 0; i<length; i++){
      ...
      if(row.c[0] && row.c[0].v && row.c[1] && row.c[1].v && row.c[2] && row.c[2].v){
        ...
        figura_grid.appendChild(product_card);
        product_count++;

        
        if(product_count >= 6){  
          if(length > 7){
            figura_grid = document.createElement('div');
            figura_grid.className = 'product-grid swiper-slide';
            figura_wrapper.appendChild(figura_grid);
            product_count = 0;
          }
