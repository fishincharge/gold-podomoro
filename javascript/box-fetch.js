...
fetch(FULL_URL)
  ...
  .then(rep => {
    ...
    let box_wrapper = document.getElementById('box-wrapper');

    let box_grid = document.createElement('div');
    box_grid.className = 'product-grid swiper-slide';
    box_wrapper.appendChild(box_grid);
    ...
    for(let i = 0; i<length; i++){
      ...
      if(row.c[0] && row.c[0].v && row.c[1] && row.c[1].v && row.c[2] && row.c[2].v){
        ...
        box_grid.appendChild(product_card);
        product_count++;

        
        if(product_count >= 6){  
          if(length > 7){
            box_grid = document.createElement('div');
            box_grid.className = 'product-grid swiper-slide';
            box_wrapper.appendChild(box_grid);
            product_count = 0;
          }
